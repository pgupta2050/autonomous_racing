...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) 
 { 
    ...
    OnlineData track_x; 
    OnlineData track_y; 
    OnlineData trackwidth; 
    BMatrix acadodata_M1;
    acadodata_M1.read( "mpc_tc_data_acadodata_M1.txt" );
    BMatrix acadodata_M2;
    acadodata_M2.read( "mpc_tc_data_acadodata_M2.txt" );
    ...
    acadodata_f1 << dot(x) == cos((atan(6.36363636363636353543e-01*tan(theta))+phi))*v;
    acadodata_f1 << dot(y) == sin((atan(6.36363636363636353543e-01*tan(theta))+phi))*v;
    acadodata_f1 << dot(phi) == 1/1.75000000000000000000e+00*tan(atan(6.36363636363636353543e-01*tan(theta)))*v;
    acadodata_f1 << dot(v) == a;

    OCP ocp1(0, 0.5, 5);
    ocp1.minimizeLSQ(acadodata_M1, acadodata_f2);
    ocp1.minimizeLSQEndTerm(acadodata_M2, acadodata_f3);
    ocp1.subjectTo((pow((-ox+x),2.00000000000000000000e+00)+pow((-oy+y),2.00000000000000000000e+00)-pow(d_safe,2.00000000000000000000e+00)) >= 0.00000000000000000000e+00);
    ocp1.subjectTo((pow((-track_x+x),2.00000000000000000000e+00)+pow((-track_y+y),2.00000000000000000000e+00)-pow(trackwidth,2.00000000000000000000e+00)) <= 0.00000000000000000000e+00);
    ocp1.subjectTo((-1.00000000000000000000e+01) <= a <= 1.00000000000000000000e+01);
    ocp1.subjectTo(0.00000000000000000000e+00 <= v <= 1.00000000000000000000e+02);
    ...
    uint export_flag;
    export_flag = ExportModule1.exportCode( "tracking_MPC_allConstr" );
