...
% Collision avoidance params init
d_safe = 0.5;
xObs = 2.5 ;      yObs = 1.0;
input.od = ones(6,6);
...
%     tic
    ...
    % pass the onlide data to mpc inpout :obstacle info, future map points and trackwidth
    input.od = [obstacle, xcMap, ycMap, trackwidth/2*ones(N+1,1)];
    
    % Solve NMPC OCP
    input.x0 = state_sim(end,:);
    
    % Run controller
    output = tracking_MPC_allConstr(input);
